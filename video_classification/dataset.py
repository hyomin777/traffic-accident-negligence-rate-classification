...
class BaseTrafficAccidentDataset(Dataset):
    ...
    def _load_annotations(self):
        ...
        for json_file in self.annotation_dir.glob("*.json"):
            ...
            video_point_of_view = int(
                data['video'].get('video_point_of_view', 3))
            ...
            accident_place_feature = int(
                data['video'].get('accident_place_feature', 0))

            vehicle_a_progress = int(
                data['video'].get('vehicle_a_progress_info', 0))
            vehicle_b_progress = int(
                data['video'].get('vehicle_b_progress_info', 0))
        ...
        logger.info(f"Num of Unavailable Data: {unavailable_data_cnt}")
    ...
    def _load_and_preprocess_frame(self, frame, apply_transform=True):
        try:
            ...
            return torch.zeros((3, *IMAGE_SIZE[::-1])), torch.zeros((20, 6))
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if total_frames <= 0:
                ...
                raise ValueError(
                    f"Invalid frame count for video: {video_path}")
            ...
            for i in frame_indices:
                ...
                if not ret:
                    ...
                    if consecutive_failures >= max_consecutive_failures:
                        logger.warning(
                            f"Too many consecutive frame read failures in video: {video_path}")
                        break
                    continue
            ...
            if len(frames) < self.max_frames:
                ...
                yolo_tensors.extend(
                    [last_yolo] * (self.max_frames - len(yolo_tensors)))
            ...
            return {
                'frames': torch.zeros((self.max_frames, 3, *IMAGE_SIZE[::-1])),
                'yolo_detections': torch.zeros((self.max_frames, 20, 6)),
                'negligence_category': sample['negligence_category'],
                'metadata': torch.tensor([
                    sample['accident_type'],
                    sample['accident_place'],
...
class TrainDataset(BaseTrafficAccidentDataset):
    ...
    def _process_frame(self, frame):
        try:
            ...
            logger.error(f"Error in train frame processing: {str(e)}")
            return torch.zeros((3, *IMAGE_SIZE[::-1])), torch.zeros((20, 6))
...
class ValDataset(BaseTrafficAccidentDataset):
    def _process_frame(self, frame):
        try:
            ...
            logger.error(f"Error in validation frame processing: {str(e)}")
            return torch.zeros((3, *IMAGE_SIZE[::-1])), torch.zeros((20, 6))
